...
**П5. Что можно проверить самому — считаем сами.** Изменённые файлы и diff для ревью и слияния — из git worktree. Проверки — `CheckRunner`. Длительности — часы Skaro. Поток агента — только повествование.
...
**Codex — `codex app-server`** (решение принято): закреплённая версия `@openai/codex` с докачкой. Проверка схемы (П7) остаётся как защита при обновлении закреплённой версии. Используем stdio и стабильную часть v2, без `experimentalApi`.
